  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`GIFTED MD UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeCommandPrefix} *ʀᴀᴍ:* ${formatp(os.totalmem() - os.freemem())}
${menuThemeCommandPrefix} *ᴛɪᴍᴇ-ɴᴏᴡ:* ${currentTime}
${menuThemeCommandPrefix} *ᴅᴀᴛᴇ-ᴛᴏᴅᴀʏ:* ${currentDate}
${menuThemeCommandPrefix} *ᴄᴏᴍᴍᴀɴᴅs:* ${commands.length}
${menuThemeCommandPrefix} *ᴛʜᴇᴍᴇ:* Gifted
${menuThemeFooter}                         
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    const { key: messageKey } = await context.reply("*ɢɪғᴛᴇᴅ ᴍᴅ...*");
    const endTime = new Date().getTime();
    const pingTime = endTime - startTime;
    await context.send(
      `*ɢɪғᴛᴇᴅ-ᴍᴅ sᴘᴇᴇᴅ: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*ɢɪғᴛᴇᴅ-ᴍᴅ ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "Gifted MD Running Since",
        body: message,
        footerText: "Gifted MD 2024",
    ...
    try {
      ...
      let _0x140248 = {
        ...
        contextInfo: {
          externalAdReply: {
            title: "Gifted Tech",
            body: "Tap to Message Owner.",
            renderLargerThumbnail: true,
            thumbnailUrl: "https://telegra.ph/file/a202f454c9532c3f5b7f8.jpg",
            thumbnail: log0,
            mediaType: 1,
            mediaUrl: "",
            sourceUrl:
              "https://wa.me/254762016957?text=Hello+Gifted+Tech!",
          },
        },
      };
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("GIFTED-MD_FANCY_TEXT")
        ) {
