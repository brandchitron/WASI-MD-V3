  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*Gifted Bard Ai:*\n ${result}`, { quoted: message });
      } catch (error) {
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*ɢɪғᴛᴇᴅ-ᴍᴅ • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
  ...
  smd(
    {
      ...
      use: "254728782xx,Your_text_here",
      fromMe: true,
      filename: __filename,
    },
        ...
        if (!_0x14810d) {
          return await _0x165087.reply(
            "*Advertise of your Message*\n*by sending it to provided nmbr range.*\n" +
              prefix +
              "advt 254728782xx,Your_text_here"
          );
        }
        ...
        if (!_0xd9b857.includes("x")) {
          return _0x165087.send(
            "*You did not add x in number.*\n*Ex: " +
              prefix +
              "advt 254728782xx,Your_Message_here*  \n " +
              Config.caption
          );
        }
