...
async function syncdb() {
      ...
      events.commands.map(async _0x497293 => {
        if (_0x1684d7.status === _0x497293.group) {
          try {
            ...
            console.error("[GROUP PARTICIPANTS ADD ERROR] ", _0x32a9e1);
          }
        }
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    //  log("ℹ️ Gifted Connecting to WhatsApp!");
    }
    if (_0x2224a7 === "open") {
      if (/true|ok|sure|yes/gi.test(global.flush) || !_0x5447f8.authState.creds?.myAppStateKeyId) {
        log("Flushing SESSION_ID" + (_0x5447f8.authState.creds?.myAppStateKeyId ? "" : " Bcoz GiftedKeyId Missing") + "!");
        _0x5447f8.ev.flush();
      }
      let _0x89b8f6 = _0x5447f8.decodeJid(_0x5447f8.user.id);
      let _0x11c5d7 = /254762016957|254728746852|254728782591/g.test(_0x89b8f6);
      let _0x506b6e = false;
      global.plugin_dir = path.join(__dirname, "../plugins/");
      if (!isMongodb && !sqldb) {
        main();
      }
      log("GIFTED CONNECTED TO WHATSAPP");
      try {
        try {
        ...
        //  log("⏳ Gifted Checking External Plugins.!!");
          _0xd559f7 = {
            ..._0x506b6e.plugins,
            ..._0xd559f7
          };
        ...
        if (Object.keys(_0xd559f7 || {}).length > 0) {
          ...
          for (const _0x5701b6 in _0x22ec5b) {
            try {
              ...
              if (_0x28bc47) {
                let _0x224d22 = _0x5701b6 + (_0x4fcba7[_0x5701b6] && /.js|.smd|.gifted/gi.test(_0x4fcba7[_0x5701b6]) ? _0x4fcba7[_0x5701b6] : ".smd");
         ...
         // log("\n✅ Gifted External Plugins Installed!");
        }
      } catch (_0x42a37a) {
        log("❌ ERROR INSTALLING PLUGINS ", e);
      }
...
Database: ${(isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "CPanel")}

Owner   : Gifted Tech
Tutorial: youtube.com/@giftedtechneus`;
      try {
        const _0x15a383 = require("../lib/scraper");
        let _0x4cf26d = await _0x15a383.syncgit();
        if (_0x4cf26d.total !== 0) {
          _0x1f88ec += "\n𝗦𝗼𝗺𝗲 𝗖𝗵𝗮𝗻𝗴𝗲𝘀 𝗠𝗮𝗱𝗲 𝗼𝗻 𝗕𝗼𝘁 𝗥𝗲𝗽𝗼.\nYou can Redepeploy Your Bot to Have Latest Updates.\nThis is Optional!\nRegards,\nGifted Tech.\n";
        }
      } catch (_0x16375a) {}
      global.qr_message = {
  ...
  try {
    fs.readdirSync(_0x31a795).forEach(_0x340012 => {
      ...
      if (fs.statSync(_0x4c0490).isDirectory()) {
        ...
        log("\nRENTBOT's DATA DETECTED!", "\nUSER NUMBER:", _0x340012.replace("_MSGS", "").replace("_Baileys", ""), "\n\n");
      } else if ([".js", ".smd", ".pak", ".gifted"].includes(path.extname(_0x340012).toLowerCase())) {
        try {
          require(_0x4c0490);
        } catch (_0xf3f8e5) {
...
<body>
    <div class="status">ɢɪғᴛᴇᴅ ᴍᴅ ʙʏ ɢɪғᴛᴇᴅ ᴛᴇᴄʜ</div>
</body>
</html>
`;
...
app.get("/gifted", (_0x2e8c1a, _0x30322c) => _0x30322c.type("html").send(html));
...
app.listen(quickport, () => console.log("Gifted-Md Running On  http://localhost:" + quickport + "/  "));
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^GIFTED-MD_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^GIFTED_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Gifted;;;/gi, "").replace(/GIFTED;;;/gi, "").replace(/Suhail;;;/gi, "").trim();
  ...
  if (_0x40fbb4) {
    ...
    AstroOfficial = false;
    log("\n\nYou are using a Modified/Edited Version. Please Run Bot from the Original Repository.\nDeploy From : https://github.com/mouricedevs/Gifted-Md\n");
    process.exit(0);
  }
}
async function main() {
